...
#main
done = whatsbeendon("all_features_training_fakelarge2.csv")
outpath = "./"
...
if len(done) == 0:
    with open(os.path.join(outpath, "all_features_training_fakelarge3.csv"), "a") as out:
        seq = ("pid,type,bias_count, assertives_count, factives_count, hedges_count, implicatives_count, report_verbs_count, positive_op_count, negative_op_count, wneg_count, wpos_count, wneu_count, sneg_count, spos_count, sneu_count, TTR,vad_neg,vad_neu,vad_pos,FKE,SMOG,stop,wordlen,WC,NB_pobj,NB_psubj,quotes,Exclaim,AllPunc,allcaps",",".join(pos_tags),",".join(liwc_cats), "TTR_title,vad_neg_title,vad_neu_title,vad_pos_title,FKE_title,SMOG_title,stop_title,wordlen_title,WC_title,NB_pobj_title,NB_psubj_title,quotes_title,Exclaim_title,AllPunc_title,allcaps_title",",".join(pos_tags_titles),",".join(liwc_cats_title),"bias_count_title, assertives_count_title, factives_count_title, hedges_count_title, implicatives_count_title, report_verbs_count_title, positive_op_count_title, negative_op_count_title, wneg_count_title, wpos_count_title, wneu_count_title, sneg_count_title, spos_count_title, sneu_count_title")
        out.write(",".join(seq)+"\n")

path = "D:/RPI Research/Fake vs Real News/Large Scale/Data/Text/Content/Young Conservatives/"
title_path = "D:/RPI Research/Fake vs Real News/Large Scale/Data/Text/Title/Young Conservatives/"
